...
// ── Create/Update Payload Normalizer ──
// The backend may also expect flat fields for create/update payloads.
// We normalize outgoing payloads from the frontend's nested format to the backend's flat format.

export function toBackendCategoryPayload(data: any): any {
  // If the backend expects flat fields, transform; otherwise pass through
  const payload: any = { ...data };
  if (data.name && typeof data.name === "object") {
    payload.nameAr = data.name.ar;
    payload.nameEn = data.name.en;
    delete payload.name;
  }
  if (data.description && typeof data.description === "object") {
    payload.descriptionAr = data.description.ar;
    payload.descriptionEn = data.description.en;
  ...
  if ("isAvailable" in data) {
    payload.available = data.isAvailable;
    delete payload.isAvailable;
  }
  if ("isVisible" in data) {
    payload.visible = data.isVisible;
    delete payload.isVisible;
  }
  if ("sortOrder" in data) {
    payload.order = data.sortOrder;
    delete payload.sortOrder;
  }
  return payload;
}

export function toBackendProductPayload(data: any): any {
  const payload: any = { ...data };
  if (data.name && typeof data.name === "object") {
    payload.nameAr = data.name.ar;
    payload.nameEn = data.name.en;
  ...
  // pricingModel -> pricingType if backend expects it
  // priceHalala stays as is (or map to price)
  return payload;
}

export function toBackendOptionGroupPayload(data: any): any {
  const payload: any = { ...data };
  if (data.name && typeof data.name === "object") {
    payload.nameAr = data.name.ar;
    payload.nameEn = data.name.en;
...
export function toBackendOptionPayload(data: any): any {
  const payload: any = { ...data };
  if (data.name && typeof data.name === "object") {
    payload.nameAr = data.name.ar;
    payload.nameEn = data.name.en;
