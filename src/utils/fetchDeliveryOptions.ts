...
export const normalizeSlots = (
  ...
  return source
    .map((rawWindow, index) => {
      const window = isRecord(rawWindow)
        ? readString(rawWindow.window) ||
          readString(rawWindow.value) ||
          readString(rawWindow.deliveryWindow)
        : readString(rawWindow);
      if (!window) return null;

      // The subscription quote service assigns this ID to string-based
      // `delivery_windows` settings. It is an API identifier, not a display
      // value: deriving `delivery-${window}` makes every selected slot fail
      // backend validation.
      const fallbackId =
        type === "delivery"
          ? `delivery_slot_${index + 1}`
          : `pickup_slot_${index + 1}`;
      const id = isRecord(rawWindow)
        ? readString(rawWindow.id) || readString(rawWindow.slotId) || fallbackId
        : fallbackId;
      ...
      return {
        ...
        type,
        window,
        label: isRecord(rawWindow)
          ? readLocalized(rawWindow.label) || window
          : window,
      };
    })
    .filter((slot): slot is DeliverySlotOption => slot !== null);
};
...
export const fetchDeliveryOptions =
  async (): Promise<DeliveryOptionsResponse> => {
    ...
    const settings = (settingsResponse.data?.data || {}) as Record<
      string,
      unknown
    >;
