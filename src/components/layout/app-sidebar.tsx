...
export function AppSidebar({ ...props }: React.ComponentProps<typeof Sidebar>) {
  ...
  const navMain = filterNavItemsForRole(NavLinksData.navMain, role);
  const navSecondary = NavLinksData.navSecondary.filter((item) => {
    if (item.url === "#") return true;
    return filterNavItemsForRole([item], role).length > 0;
  });
