...
## Remaining Dashboard Implementation Briefs

### Menu Categories

**Auth / Roles**

`src/routes/dashboardMenu.js` protects this route with `dashboardAuthMiddleware` and `dashboardRoleMiddleware(["admin", "superadmin"])`.
...
// body
{
  name: { ar: string; en: string }        // required
  key?: string                            // optional; omit on create so backend generates it
  ...
  sortOrder?: number
  ui?: {
    cardVariant?: "meal_builder" | "light_collection" | "sandwich_collection" | "addon_collection"
  }
  branchIds?: string[]
}
```
...
// body
{
  name: { ar: string; en: string }        // required by normalizer after merge
  description?: { ar: string; en: string }
  imageUrl?: string
  isActive?: boolean
  isVisible?: boolean
...
The dashboard still requires manual `key` input in category create forms, while the backend generates keys when omitted and rejects key changes with `IMMUTABLE_KEY`. The dashboard types, schema, form, and payload mapper do not include `ui.cardVariant`.

**What the dashboard team must do**

1. Edit `src/types/menuTypes.ts` and add `ui.cardVariant` to `MenuCategory`, `CreateMenuCategoryPayload`, and `UpdateMenuCategoryPayload`; make `CreateMenuCategoryPayload.key` optional.
2. Edit `src/lib/validations/menuCategorySchema.ts` so `key` is optional on create and add `ui.cardVariant`.
3. Edit `src/components/pages/menu/categories/MenuCategoryFormFields.tsx` to remove required manual key input on create, show generated key read-only on edit, and add a dropdown for `ui.cardVariant`.
4. Edit `src/utils/menuPayloadMappers.ts` so create omits empty `key` and create/update send `ui.cardVariant`.
5. Edit category create/update route defaults in `src/routes/_protected/menu/categories/create.tsx` and `src/routes/_protected/menu/categories/$categoryId/update.tsx` to initialize and hydrate the `ui.cardVariant` field.
...
// body
{
  ...
  sortOrder?: number
  ui?: {
    cardVariant?: "standard" | "premium" | "large_salad" | "addon"
    badge?: string
    ctaLabel?: string
    imageRatio?: string
  }
...
// body
{
  categoryId: string                      // required by normalizer after merge
  name: { ar: string; en: string }        // required by normalizer after merge
  itemType?: string
  description?: { ar: string; en: string }
  imageUrl?: string
...
The dashboard still requires manual `key` input for products even though backend-generated keys are supported. Product `ui.cardVariant`, `ui.badge`, `ui.ctaLabel`, and `ui.imageRatio` are missing from dashboard types, forms, and payload mappers.

**What the dashboard team must do**

1. Edit `src/types/menuTypes.ts` and add product `ui` fields to `MenuProduct`, `CreateMenuProductPayload`, and `UpdateMenuProductPayload`; make `CreateMenuProductPayload.key` optional.
2. Edit `src/lib/validations/menuProductSchema.ts` so `key` is optional on create and add `ui.cardVariant`, `ui.badge`, `ui.ctaLabel`, and `ui.imageRatio`.
3. Edit `src/components/pages/menu/products/MenuProductFormFields.tsx` to remove required manual key input on create, show generated key read-only on edit, and render controls for all product `ui` fields.
4. Edit `src/utils/menuPayloadMappers.ts` so create omits empty `key` and create/update send the nested `ui` object.
5. Edit product create/update route defaults in `src/routes/_protected/menu/products/create.tsx` and `src/routes/_protected/menu/products/$productId/update.tsx` to initialize and hydrate product `ui`.
6. Edit `src/utils/menuResponseNormalizers.ts` if it drops `ui` from product responses.
...
// body
{
  ...
  sortOrder?: number
  ui?: {
    displayStyle?: "chips" | "radio_cards" | "checkbox_grid" | "dropdown" | "stepper"
  }
}
```
...
The dashboard still requires manual `key` input for option groups even though the backend can generate it. The dashboard cannot send or display `ui.displayStyle`.

**What the dashboard team must do**

1. Edit `src/types/menuTypes.ts` and add `ui.displayStyle` to `MenuOptionGroup`, `CreateMenuOptionGroupPayload`, and `UpdateMenuOptionGroupPayload`; make `CreateMenuOptionGroupPayload.key` optional.
2. Edit `src/lib/validations/menuOptionGroupSchema.ts` so `key` is optional on create and add `ui.displayStyle`.
3. Edit `src/components/pages/menu/option-groups/MenuOptionGroupFormFields.tsx` to remove required manual key input on create, show generated key read-only on edit, and add a dropdown for `ui.displayStyle`.
4. Edit `src/utils/menuPayloadMappers.ts` so create omits empty `key` and create/update send `ui.displayStyle`.
5. Edit option-group create/update route defaults in `src/routes/_protected/menu/option-groups/create.tsx` and `src/routes/_protected/menu/option-groups/$groupId/update.tsx` to initialize and hydrate `ui.displayStyle`.
6. Edit `src/utils/menuResponseNormalizers.ts` if it drops `ui` from option-group responses.

**Priority**

P0

### One-Time Order Detail Fulfillment

**Auth / Roles**

`src/routes/dashboardOrders.js` protects `/api/dashboard/orders/:orderId/actions/:action` with `dashboardAuthMiddleware` and `dashboardRoleMiddleware(["admin", "kitchen", "courier"])`; `dashboardRoleMiddleware` allows `superadmin`. For pickup `fulfill`, `orderOpsTransitionService.actionRoles` allows `superadmin`, `admin`, and `kitchen`.
...
`src/components/pages/one-time-orders/OneTimeOrderDetail.tsx` renders a pickup-code input and disables confirm until a code is entered for `fulfill`. Backend docs and services say one-time pickup fulfillment is visual verification only and does not require `payload.pickupCode`.

**What the dashboard team must do**

1. Edit `src/components/pages/one-time-orders/OneTimeOrderDetail.tsx` so `fulfill` does not set `requiresPickupCode`.
2. Edit `src/components/pages/one-time-orders/OneTimeOrderDetail.tsx` to remove the pickup-code input from the fulfill confirmation dialog.
3. Edit `src/components/pages/one-time-orders/OneTimeOrderDetail.tsx` so the confirm button is not disabled by missing `pickupCode`.
4. Edit `src/utils/fetchOneTimeOrders.ts` so `fulfillOneTimeOrder` does not send `pickupCode`; keep `reason` and `notes`.
5. Verify the detail page displays the backend pickup code from `data.context.pickupCode` or `data.pickup.pickupCode` for visual comparison only.
...
### Pickup Branch Editing Endpoint

**Auth / Roles**

Not available. Pickup locations are currently read from the `pickup_locations` setting through `GET /api/dashboard/settings`, which is admin/superadmin protected by `src/routes/admin.js`.

**Request**

No backend endpoint exists for creating, updating, deleting, activating, or reordering pickup branches.

```ts
// no request contract exists
{}
```

**Response shape**

No dedicated response shape exists. The only available shape is embedded in settings:
...
The dashboard can only show pickup branches read-only. `PATCH /api/dashboard/settings` rejects `pickup_locations` because `normalizeSettingsPatchPayloadOrThrow` does not allow that key.

**What the dashboard team must do**

1. Keep `src/routes/_protected/pickup-branches/index.tsx` read-only until a backend write endpoint exists.
2. After backend adds branch write APIs, create or edit `src/types/pickupTypes.ts` with the new request and response fields.
3. After backend adds branch write APIs, edit `src/utils/fetchPickupBranches.ts` to call the dedicated endpoints instead of only deriving data from settings.
4. After backend adds branch write APIs, add create/edit/delete UI to `src/routes/_protected/pickup-branches/index.tsx`.

**Priority**

P1

### Self-Service Dashboard Password Change Endpoint

**Auth / Roles**

Not available. `GET /api/dashboard/auth/me` uses `dashboardOptionalAuthMiddleware`; `POST /api/dashboard/dashboard-users/:id/reset-password` is under `src/routes/admin.js` after `dashboardRoleMiddleware(["admin"])`, with `superadmin` allowed by middleware logic.

**Request**

No self-service password-change endpoint exists for the current user.
...
The existing admin reset endpoint is:
...
// body
{
  password: string // required; at least 12 chars, uppercase, lowercase, number, symbol, not weak default
}
```
...
The profile page cannot offer a true current-password self-service change for all dashboard users. It can only reuse the admin reset endpoint, which is admin/superadmin only.

**What the dashboard team must do**

1. Keep password reset in `src/routes/_protected/profile/index.tsx` limited to admin/superadmin until backend adds a self-service endpoint.
2. After backend adds a self-service endpoint, edit `src/types/auth.ts` or create `src/types/profileTypes.ts` with `currentPassword`, `newPassword`, and the exact response shape.
3. After backend adds a self-service endpoint, create `src/utils/fetchProfile.ts` or extend `src/lib/authApi.ts` to call the new endpoint.
4. After backend adds a self-service endpoint, create `src/hooks/useProfileMutation.ts` or extend the existing auth hook.
5. After backend adds a self-service endpoint, edit `src/routes/_protected/profile/index.tsx` to enable password change for cashier, kitchen, courier, admin, and superadmin according to the new backend middleware.
