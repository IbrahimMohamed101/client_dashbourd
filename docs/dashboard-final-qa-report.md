...
## Executive Summary
- Overall status: Ready with accepted known issues, conditional on accepting the remaining QA data caveats
- Tested branch: main
- Tested commit: b5418885a1cdcf09162666a2d147064934a68bd6 plus focused Issue #15 retest report update
...
- Browser/device coverage: authenticated Chrome headless route smoke for admin across desktop/tablet/mobile, corrected admin desktop/mobile route pass, dynamic detail route pass after one frontend P1 fix, role access checks for superadmin/courier/kitchen/cashier, dark/light theme checks, RTL route rendering checks, and focused Issue #18/#19/#20 retest.
- Runtime API coverage: authenticated dashboard/courier/settings/promo/zones probes and safe mutation checks against hosted QA seed data on 2026-07-03; focused retest ran against `https://basicdiet145.onrender.com`.
- Date: 2026-07-03
- Worktree note: pre-existing unrelated graphify output and `docs/BACKEND_CONTRACT_ALIGNMENT_PLAN.md` deletion were not touched. Relevant focused retest change is this QA report.
...
| Delivery allowedActions | admin/superadmin/courier | Inspect backend action metadata | GET `/api/courier/deliveries/today`; GET `/api/courier/orders/today` | none | 2026-07-03 focused retest: deliveries 200 with 5 non-actionable rows, empty `allowedActions`/`allowedActionIds`, zero legacy `can*` true conflicts, and zero action-id mismatches; orders 200 with 0 rows | Frontend renders from canonical `allowedActions`; no duplicate executable action buttons observed; no safe action row existed to execute | Fixed original conflict; action execution Blocked by Missing Data |
...
| Promo in-use archive | admin | Archive seeded in-use promo | DELETE `/api/dashboard/promo-codes/:id` | none | 2026-07-03 focused retest: `includeDeleted=true` returned 200 with 2 rows and 1 archived row, but `PMQAUSED10` and `PMQAUNUSED25` were absent | Cannot verify `409 PROMO_IN_USE` until Issue #21 deployed seed rows exist; frontend not failed for missing seed | Blocked by Missing Data |
...
|---|---|---|---|---|---|---|---|---|---|---|
| RUNTIME-001 | P1 Critical | Backend / Integration / Test Data | `/delivery` | GET `/api/courier/deliveries/today`; GET `/api/courier/orders/today` | Focused retest after Issue #18 backend deploy: admin/superadmin/courier all received 200. Deliveries returned 5 non-actionable rows with empty canonical `allowedActions`/`allowedActionIds`, zero `allowedActionIds` mismatches, and zero legacy `can*` true conflicts; orders returned 0 rows. | Log in as admin/superadmin/courier, call courier today endpoints, inspect canonical action metadata, and open `/delivery`. | Non-actionable synthetic rows expose no executable canonical actions and no conflicting legacy flags; actionable rows expose structured canonical actions. | Original `canCancel:true` with empty canonical actions conflict is fixed. No actionable row existed, so method/endpoint execution, action toast, disabled action behavior, and invalidation could not be verified. | Provide at least one safe actionable deployed delivery/order row to complete action execution retest, or accept the remaining action-execution gap as a QA data caveat. | Fixed for original conflict; Blocked by Missing Data for action execution |
...
| QA-004 | P1 Critical | Backend / Integration / Test Data | /delivery, /operations courier board | courier actions | Backend canonical/legacy conflict is fixed for deployed `/api/courier/*/today` data, but no actionable delivery/order row existed for mutation execution. | 2026-07-03 focused retest: admin/superadmin/courier received 200; deliveries returned 5 rows with empty `allowedActions`/`allowedActionIds`, zero legacy `can*` true conflicts, and zero action-id mismatches; orders returned 0 rows; `/delivery` rendered with no console/network errors. | Load `/delivery` with deployed courier data and inspect API response/actions. | Non-actionable rows have empty canonical actions and no legacy conflicts; action-capable rows expose structured `allowedActions`. | Original conflicting `canCancel:true` state is gone. Action buttons/mutations could not be exercised because deployed seed has no actionable row. | Add one safe actionable delivery/order row for final method/endpoint/toast/invalidation retest, or accept action execution as a QA data caveat. | Yes, unless accepted as data caveat | Fixed original conflict; action execution Blocked by Missing Data |
| QA-005 | P1 Critical | Integration / Runtime QA / Test Data | /promo-codes | archive | Unused promo soft archive is runtime verified; in-use `409 PROMO_IN_USE` remains blocked by missing deployed Issue #21 seed rows. | 2026-07-03 focused retest: GET `/api/dashboard/promo-codes?includeDeleted=true&limit=100` returned 200 with 2 rows and 1 archived row; `PMQAUSED10` and `PMQAUNUSED25` were both absent. | Log in as admin, list promos with `includeDeleted=true`, then attempt archive on a known in-use promo if seed exists. | Unused promo archives; in-use promo returns `409 PROMO_IN_USE`; archived promos appear with `includeDeleted=true`. | Archived-list path still works; in-use conflict path could not run because Issue #21 deployed seed rows are missing. | Add/restore `PMQAUSED10` and `PMQAUNUSED25`, then retest in-use archive conflict. Do not fail frontend for missing seed. | Yes, unless accepted as data caveat | Blocked by Missing Data - Issue #21 |
...
- QA-004 / RUNTIME-001: Original canonical/legacy conflict is fixed in deployed data. Remaining gap is test data: provide one safe actionable courier row if action execution, backend method/endpoint metadata, disabled-state behavior, toasts, and invalidation must be verified before refactor.
...
- QA-004: Retest `/delivery` action rendering and mutations only after deployed seed data includes at least one actionable courier delivery/order row. Current non-actionable rows are consistent.
- QA-005 / Issue #21: Add/restore `PMQAUSED10` and `PMQAUNUSED25`, then retest `409 PROMO_IN_USE`; `includeDeleted=true` still shows archived promos.

## Environment / Test Data Issues
- QA-001: Auth access is resolved for admin, superadmin, courier, kitchen, and cashier. Remaining focused data issues are specific: no actionable courier row in `/api/courier/*/today`, and missing Issue #21 promo seeds.
...
- QA-004: Original conflict fixed. Provide an actionable delivery/order seed only if action execution coverage is required before refactor.
- QA-010: Fixed and runtime verified.
- QA-005 / Issue #21: Restore in-use promo seed to verify `409 PROMO_IN_USE`, or accept as a QA data caveat.
...
## Risks Remaining
- Delivery action execution could not be verified because deployed `/api/courier/deliveries/today` had 5 non-actionable rows and `/api/courier/orders/today` had 0 rows; the previous legacy/canonical conflict is fixed.
- In-use promo archive `409 PROMO_IN_USE` still needs valid deployed Issue #21 promo seed rows.
...
- Refactor can start only if the delivery actionable-row gap and Issue #21 promo seed gap are accepted as QA data caveats.

## Release Recommendation
Ready with accepted known issues if product/backend accepts the remaining QA data caveats: no deployed actionable courier row for mutation execution and missing Issue #21 promo seed rows. Otherwise, wait for those test data fixtures and rerun the two focused checks before starting refactor.
