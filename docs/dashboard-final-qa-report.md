...
## Executive Summary
- Overall status: Ready to start refactor phase; no remaining P1 runtime blockers from QA-004/QA-005.
- Tested branch: main
- Tested commit: 9c3e0ee5c51f88e5cba885f5979bbc32a98b5ff4 plus focused Issue #15 retest report/update.
...
- Browser/device coverage: authenticated Chrome headless route smoke for admin across desktop/tablet/mobile, corrected admin desktop/mobile route pass, dynamic detail route pass after one frontend P1 fix, role access checks for superadmin/courier/kitchen/cashier, dark/light theme checks, RTL route rendering checks, focused Issue #18/#19/#20 retest, and final Issue #21/#22 focused retest.
- Runtime API coverage: authenticated dashboard/courier/settings/promo/zones probes and safe mutation checks against hosted QA seed data on 2026-07-03; final focused retest ran against `https://basicdiet145.onrender.com` on 2026-07-04.
- Date: 2026-07-04
- Worktree note: pre-existing unrelated graphify output and `docs/BACKEND_CONTRACT_ALIGNMENT_PLAN.md` deletion were not touched. Relevant focused retest changes are this QA report and one small frontend promo archive guard fix.
...
| npm run typecheck | Pass | 2026-07-03 focused Issue #15 retest verification: tsc --noEmit completed with exit code 0. |
| npm run lint | Pass | 2026-07-03 focused Issue #15 retest verification: eslint . completed with exit code 0. |
| npm run build | Pass | 2026-07-03 focused Issue #15 retest verification: tsc -b && vite build completed; 3809 modules transformed. |
| npm run typecheck | Pass | 2026-07-04 final Issue #15 QA-data retest verification: tsc --noEmit completed with exit code 0. |
| npm run lint | Pass | 2026-07-04 final Issue #15 QA-data retest verification: eslint . completed with exit code 0. |
| npm run build | Pass | 2026-07-04 final Issue #15 QA-data retest verification: tsc -b && vite build completed; 3810 modules transformed. |

## Final Focused Authenticated QA-Data Retest - 2026-07-04

Secure dashboard access was used through the agreed private local process. No usernames, passwords, bearer tokens, or screenshots are included here.

Frontend was served locally with `VITE_BACKEND_URL=https://basicdiet145.onrender.com`; `/api` was not included in the env base URL.

| Issue | Role Used | Route | Method + Endpoint | Payload Summary | Response Status / Body Summary | UI Result | Console / Network Errors | Final Status |
|---|---|---|---|---|---|---|---|---|
| Issue #21 / QA-005 | admin | `/promo-codes` | GET `/api/dashboard/promo-codes?includeDeleted=false`; GET `/api/dashboard/promo-codes?includeDeleted=true&limit=100` | none | 200. Normal list returned 3 rows; includeDeleted returned 4 rows. `PMQAARCHIVESAFE` and `PMQAARCHIVECONFLICT` were present before mutation. | `/promo-codes` loaded with valid admin auth; both QA codes were visible through table search. | No browser console errors, page errors, failed requests, or unrelated network errors. | Passed |
| Issue #21 / QA-005 | admin | `/promo-codes` | DELETE `/api/dashboard/promo-codes/:id`; follow-up GET `/api/dashboard/promo-codes?includeDeleted=true&limit=100` | Safe archive promo id only; no secrets | Safe promo DELETE returned 200. Follow-up includeDeleted GET returned 200 and showed `PMQAARCHIVESAFE` archived/deleted state. | Archive confirmation dialog executed from the UI; success behavior observed; archived promo remains visible when includeDeleted is enabled. | No browser console errors, page errors, failed requests, or unrelated network errors. | Fixed - Verified |
| Issue #21 / QA-005 | admin | `/promo-codes` | DELETE `/api/dashboard/promo-codes/:id`; follow-up GET `/api/dashboard/promo-codes?includeDeleted=true&limit=100` | In-use conflict promo id only; no secrets | Initial retest found frontend disabled archive for used rows, preventing the expected backend conflict path. After the targeted frontend fix, UI DELETE returned 409 with `PROMO_IN_USE`; follow-up includeDeleted GET confirmed `PMQAARCHIVECONFLICT` remained unarchived. | UI now allows backend-owned archive validation, shows a clear error toast, and does not mark the in-use promo archived. | Only expected 409 handling appeared: browser logged the failed 409 resource and API error message. No page errors or unrelated request failures. | Fixed - Verified |
| Issue #22 / QA-004 action execution gap | admin, superadmin, courier | `/delivery` | GET `/api/courier/deliveries/today`; GET `/api/courier/orders/today` | none | 200 for all roles. Deliveries returned 17 rows; orders returned 0 rows. Each role saw 5 actionable subscription delivery rows. Target row had canonical `allowedActions` `[arriving_soon, delivered, cancel]`; `allowedActionIds` matched exactly; zero action-id mismatches; zero legacy `can*` conflicts on non-actionable rows. | `/delivery` rendered for admin/superadmin/courier. Action buttons rendered from backend labels/actions with no duplicate buttons observed. | No browser console errors, page errors, failed requests, or API network failures during read/UI checks. | Fixed - Verified |
| Issue #22 / QA-004 action execution gap | courier | `/delivery` | PUT backend-provided `/api/courier/deliveries/:id/arriving-soon`; follow-up GET `/api/courier/orders/today`; GET `/api/courier/deliveries/today` | Backend-provided action endpoint/method; no reason payload required | PUT returned 200. React Query invalidation/refetch produced 2 courier GETs after mutation. | Clicked backend-provided `Arriving Soon` action button from the delivery card; success toast behavior was observed. | No browser console errors, page errors, or failed requests. | Fixed - Verified |

## Focused Authenticated Runtime Retest - 2026-07-03
...
| Issue | Role Used | Route | Method + Endpoint | Payload Summary | Response Status / Body Summary | UI Result | Console / Network Errors | Final Status |
|---|---|---|---|---|---|---|---|---|
| Issue #18 / RUNTIME-001 | admin, superadmin, courier | `/delivery` | GET `/api/courier/deliveries/today`; GET `/api/courier/orders/today` | none | 200 for all roles. Deliveries returned `status:true` with 5 rows; orders returned `status:true` with 0 rows. All 5 delivery rows had empty `allowedActions`/`allowedActionIds`, zero `allowedActionIds` mismatches, and zero legacy `can*` true conflicts. | `/delivery` rendered for admin/superadmin/courier. No duplicate executable action buttons were rendered because no canonical executable actions were present. | No browser console errors, page errors, failed requests, or API network failures. | Passed for original conflict; Blocked by Missing Data for actionable-row button/mutation execution |
| Issue #18 / RUNTIME-001 | admin, superadmin, courier | `/delivery` | PUT backend-provided `allowedActions.endpoint` | Not sent; no safe actionable row existed in deployed `/api/courier/*/today` data. | Not applicable. No disabled actions were present either. | Could not verify backend method/endpoint execution, success/error toast, or invalidation/refetch after mutation without an actionable row. | No browser console/network errors during the read-only UI pass. | Blocked by Missing Data |
| Issue #19 / RUNTIME-002 | admin | `/pickup-branches` | GET `/api/dashboard/settings` | none | 200 `status:true`; `pickup_locations` returned 1 canonical row before mutation with `id`, `name.ar/en`, `address.ar/en`, `isActive`, latitude, and longitude. | `/pickup-branches` rendered from settings data. | No browser console errors, page errors, failed requests, or API network failures. | Passed |
| Issue #19 / RUNTIME-002 | admin | `/pickup-branches` | PATCH `/api/dashboard/settings` | Only `pickup_locations`; added a safe QA branch, edited localized name/address and coordinates, toggled inactive, then restored active. | 200 for add, edit/toggle, and restore. Follow-up GET confirmed persistence. Non-`pickup_locations` settings were unchanged after each PATCH. | UI showed the branch list; empty create dialog kept 4 required fields invalid and stayed open. | No browser console/network errors. Direct API invalid-payload probe was cleaned up; frontend validation blocked empty required fields in UI. | Fixed - Verified |
| Issue #20 / RUNTIME-004 | admin | `/dashboard-users` | GET `/api/dashboard/dashboard-users?page=1&limit=20` | none | 200 `status:true`; 20 staff rows returned. | Staff list/stat cards rendered. React unique-key warning count was 0. | No browser console errors, page errors, failed requests, or API network failures. | Fixed - Verified |
| Issue #21 / QA-005 | admin | `/promo-codes` | GET `/api/dashboard/promo-codes?includeDeleted=true&limit=100` | none | 200; 2 promo rows returned, including 1 archived row. `PMQAUSED10` and `PMQAUNUSED25` were both absent. | In-use archive conflict path was not attempted because the deployed seed rows are missing. Existing `includeDeleted=true` archived-list path still works. | No frontend issue indicated by the missing seed rows. | Blocked by Missing Data |
...
|---|---|---|---|---|
| `/dashboard`, `/users`, `/users/create`, `/subscriptions`, `/subscriptions/create`, `/settings`, `/restaurant-hours`, `/promo-codes`, `/zones`, `/premium-meals`, `/pickup-branches`, `/payments`, `/packages`, `/operations`, `/one-time-orders`, `/notifications`, `/menu`, `/manual-deduction`, `/delivery`, `/dashboard-users`, `/addons`, `/accounting` | admin | Desktop dark, tablet light, mobile light first pass; corrected desktop/mobile route classifier rerun; focused Issue #20 retest | Passed | Authenticated routes rendered without redirecting to login. `/dashboard-users` warning was fixed and focused browser retest showed zero React unique-key warnings. |
...
|---|---|---|---|---|---|---|---|
| Delivery role access | admin/superadmin/courier | Load courier deliveries/orders | GET `/api/courier/deliveries/today`, GET `/api/courier/orders/today` | none | 2026-07-04 final focused retest: 200 for all roles; deliveries returned 17 rows, orders returned 0 rows | `/delivery` renders for admin/superadmin/courier | Passed |
...
| Delivery allowedActions/action execution | admin/superadmin/courier; action as courier | Inspect backend action metadata and execute safe action | GET `/api/courier/deliveries/today`; GET `/api/courier/orders/today`; PUT backend-provided `/api/courier/deliveries/:id/arriving-soon` | none for GET; no reason payload required for `arriving_soon` | 2026-07-04 final focused retest: deliveries 200 with 17 rows and 5 actionable subscription delivery rows. Target row exposed `allowedActions` and matching `allowedActionIds` `[arriving_soon, delivered, cancel]`; zero legacy conflicts. PUT returned 200 and follow-up courier GET refetches ran. | Frontend renders from canonical `allowedActions`; no duplicate executable action buttons observed; backend method/endpoint respected; toast/refetch behavior observed | Fixed and verified |
...
| Pickup branches save | admin | Add/edit active/inactive branch | PATCH `/api/dashboard/settings` | Only `pickup_locations` key; no other settings keys | 2026-07-03 focused retest: 200 add, 200 edit/toggle inactive, 200 restore active; follow-up GET persisted the safe QA branch and preserved other settings fields | `/pickup-branches` save/edit/reload passes; empty create dialog keeps required localized fields invalid | Passed |
...
| Promo in-use archive | admin | Archive seeded in-use promo | DELETE `/api/dashboard/promo-codes/:id` | promo id only | 2026-07-04 final focused retest: `PMQAARCHIVECONFLICT` present; DELETE returned 409 with `PROMO_IN_USE`; follow-up includeDeleted GET confirmed promo remained unarchived | UI now allows backend-owned archive validation, shows a clear error toast, and does not mark the in-use promo archived | Fixed and verified |
...
|---|---|---|---|---|---|---|---|---|---|---|
| RUNTIME-001 | P1 Critical | Backend / Integration | `/delivery` | GET `/api/courier/deliveries/today`; GET `/api/courier/orders/today`; PUT backend-provided `/api/courier/deliveries/:id/arriving-soon` | Final focused retest after Issue #22 seed deploy: admin/superadmin/courier all received 200. Deliveries returned 17 rows, including 5 actionable subscription delivery rows with canonical `allowedActions` `[arriving_soon, delivered, cancel]`; `allowedActionIds` matched; zero legacy `can*` conflicts on non-actionable rows. Courier UI action execution returned 200 and refetched courier queries. | Log in as admin/superadmin/courier, open `/delivery`, inspect courier today endpoints, then execute a safe subscription delivery action as courier. | Non-actionable synthetic rows expose no executable canonical actions and no conflicting legacy flags; action-capable rows expose structured `allowedActions`; UI uses backend method/endpoint and refetches after mutation. | Behavior matched expected runtime contract. | Keep backend canonical `allowedActions` as the action source and keep `/api/courier/*` endpoint family for `/delivery`. | Fixed - Verified |
| RUNTIME-002 | P1 Critical | Integration / Seed Data | `/pickup-branches` | GET/PATCH `/api/dashboard/settings` | Focused retest after Issue #19 backend deploy: GET returned canonical `pickup_locations`; PATCH with only `pickup_locations` added/edited/toggled/restored a safe QA branch with 200 responses; follow-up GET confirmed persistence and other settings were unchanged. | Log in as admin, open `/pickup-branches`, GET settings, PATCH `{ pickup_locations }`, reload, and test empty required fields in the dialog. | Current settings data can be round-tripped by the frontend contract and frontend validation clearly blocks missing localized name/address fields. | Save/edit/reload passed; empty create dialog kept 4 required fields invalid and stayed open. | Keep settings-owned pickup branch contract. Direct API accepted an invalid empty branch payload during probing; invalid row was removed, and frontend validation still blocks this path. | Fixed - Verified |
...
| RUNTIME-004 | P3 Minor | Frontend | `/dashboard-users` | UI render | Focused retest after Issue #20 frontend fix: GET `/api/dashboard/dashboard-users?page=1&limit=20` returned 200 with 20 rows; `/dashboard-users` rendered staff list/stat cards; React unique-key warning count was 0. | Log in as admin and open `/dashboard-users`. | No React console warnings. | Screen renders without the previous key warning. | Keep stable keys in dashboard-users stats and staff list card renders. | Fixed - Verified |
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| QA-001 | P1 Critical | Environment / Test Data | all protected routes | all authenticated workflows | Valid runtime access is available and authenticated QA was executed on 2026-07-03 and 2026-07-04. | Admin, superadmin, courier, kitchen, and cashier sessions authenticated; protected route smoke and focused mutations ran against hosted QA seed data. Final Issue #21/#22 seed rows were available on 2026-07-04. | Log in with secure QA access and load protected routes. | Authenticated QA account can load and mutate safe test records. | Behavior matched expected runtime setup for the focused QA scope. | Keep secure role access and safe seed rows available for future regression retests. | Yes | Closed |
...
| QA-004 | P1 Critical | Integration / Runtime QA | /delivery, /operations courier board | courier actions | Backend canonical/legacy conflict and final action execution gap are runtime verified. | 2026-07-04 final focused retest: admin/superadmin/courier received 200; deliveries returned 17 rows with 5 actionable subscription delivery rows; target row exposed canonical `allowedActions` and matching `allowedActionIds` `[arriving_soon, delivered, cancel]`; zero legacy conflicts; courier clicked `Arriving Soon`; PUT backend-provided `/api/courier/deliveries/:id/arriving-soon` returned 200; two courier GET refetches ran. | Load `/delivery` with deployed courier data as admin/superadmin/courier, inspect API response/actions, then execute a safe subscription delivery action as courier. | Non-actionable rows have empty canonical actions and no legacy conflicts; action-capable rows expose structured `allowedActions`; UI action uses backend method/endpoint and refetches. | Behavior matched expected runtime contract. | Keep `/delivery` on `/api/courier/*` and keep canonical `allowedActions` as the action source. | Yes | Fixed and verified |
| QA-005 | P1 Critical | Integration / Runtime QA / Frontend | /promo-codes | archive | Unused promo soft archive and in-use `409 PROMO_IN_USE` are runtime verified. | 2026-07-04 final focused retest: `PMQAARCHIVESAFE` and `PMQAARCHIVECONFLICT` were present. Safe promo DELETE returned 200 and includeDeleted showed archived state. In-use conflict promo DELETE returned 409 with `PROMO_IN_USE`; UI showed clear error and follow-up includeDeleted confirmed it remained unarchived. | Log in as admin, list promos, archive `PMQAARCHIVESAFE`, enable includeDeleted, then attempt archive on `PMQAARCHIVECONFLICT`. | Unused promo archives; in-use promo returns `409 PROMO_IN_USE`; archived promos appear with `includeDeleted=true`; UI does not mark the conflict promo archived. | Behavior matched expected after removing the frontend usage-count archive disable guard so backend owns conflict validation. | Keep backend-owned archive validation; do not re-disable archive solely from frontend usage counts. | Yes | Fixed and verified |
...
| QA-010 | P1 Critical | Integration / Seed Data | /pickup-branches | PATCH /api/dashboard/settings | Settings-backed pickup branch save is runtime verified after Issue #19 backend deploy. | 2026-07-03 focused retest: GET returned canonical branch data; PATCH payloads contained only `pickup_locations`; add/edit/toggle/restore returned 200; follow-up GET persisted the safe QA branch and preserved all other settings fields. | GET settings, append/edit/toggle a valid pickup branch, PATCH `{ pickup_locations: nextBranches }`, reload, and test empty required fields in the UI dialog. | Existing settings can be round-tripped and only pickup_locations is patched. | Behavior matched expected settings contract; UI validation kept 4 required localized fields invalid on empty save. | Keep contract and frontend validation. Direct API invalid-payload acceptance should be reviewed separately if backend-side validation is required. | Yes | Fixed and verified |
...
| QA-012 | P3 Minor | Frontend | /dashboard-users | render list/cards | React unique-key warning is fixed after Issue #20. | 2026-07-03 focused browser retest: `/dashboard-users` rendered staff list/stat cards, GET `/api/dashboard/dashboard-users?page=1&limit=20` returned 200 with 20 rows, and React unique-key warning count was 0. | Open `/dashboard-users` as admin. | No React console warnings. | Behavior matched expected. | Keep stable keys in dashboard-users stats and staff list card render. | No | Fixed and verified |
...
- QA-012: Fixed and runtime verified. `/dashboard-users` stats and staff list card rendering now use stable keys and the focused browser retest showed zero React unique-key warnings.
- QA-008: Remove or implement stale direct create/update routes.

## Backend Issues To Fix
- QA-010 / RUNTIME-002: Fixed and runtime verified. Settings-backed pickup branch add/edit/toggle/restore now round-trips with only `pickup_locations`.
...
- QA-004 / Issue #22: Fixed and runtime verified. `/delivery` renders backend canonical `allowedActions`; one safe subscription delivery action executed through backend-provided method/endpoint and refetched courier data.
- QA-005 / Issue #21: Fixed and runtime verified. Safe promo archive and in-use `409 PROMO_IN_USE` conflict both pass against deployed QA rows.

## Environment / Test Data Issues
- QA-001: Auth access is resolved for admin, superadmin, courier, kitchen, and cashier. The previously missing Issue #21 promo seed rows and Issue #22 actionable courier row are now deployed and verified.
...
- QA-004: Fixed and runtime verified, including action execution/toast/refetch.
- QA-010: Fixed and runtime verified.
- QA-005 / Issue #21: Fixed and runtime verified after deployed promo seeds and frontend archive guard fix.
...
| Promo Codes | /api/dashboard/promo-codes | GET/POST | Source converts fixed SAR to halalas | No active blocker | Runtime verified for list/archive focus paths; create fixed/percentage previously passed. |
| Promo Codes | /api/dashboard/promo-codes/:id | GET/PUT/DELETE | Contract resolved and runtime verified for DELETE archive/conflict | No active blocker | 2026-07-04: safe promo DELETE soft-archived; in-use promo DELETE returned `409 PROMO_IN_USE`; `includeDeleted=true` showed archived promo. |
...
| Delivery | /api/courier/deliveries/today, /api/courier/orders/today | GET | Contract resolved and runtime verified | No active blocker | Backend confirms `/delivery` may use `/api/courier/*`; admin/superadmin/courier received 200 and actionable subscription delivery rows on 2026-07-04. |
| Delivery | /api/courier/deliveries/:id/:action, /api/courier/orders/:id/:action or backend action endpoint | PUT or backend-provided method | Contract resolved and runtime verified for subscription delivery action execution | No active blocker | 2026-07-04: frontend used backend-provided `PUT /api/courier/deliveries/:id/arriving-soon`, received 200, showed toast behavior, and refetched courier data. |
...
| QA-005 / Issue #21 | `src/components/pages/promo-codes/promo-codes-columns.tsx` | Removed the frontend-only usage-count archive disable guard so in-use archive attempts reach the backend and surface `409 PROMO_IN_USE` through the existing error toast. Archived rows and pending mutations remain disabled. | Narrow archive-action guard fix; no endpoint, payload, query key, invalidation, or archive semantics changed. Backend remains the source of truth for in-use conflict validation. |

## Risks Remaining
- A direct API invalid pickup-branch payload was accepted during probing and immediately cleaned up; frontend validation blocks empty required localized fields, but backend-side validation strictness should be reviewed separately if required.
- QA-002 dependency vulnerabilities remain open for a separate dependency update track if this dashboard is release-bound.
- QA-008 stale redirect-only routes remain a P3 cleanup item, not a refactor blocker.

## Release Recommendation
Ready to start refactor phase. Issues #21 and #22 are fixed and verified; there are no remaining P1 runtime blockers from the final focused QA-data retest.
...
| /promo-codes | src/routes/_protected/promo-codes/index.tsx | Promo Codes | admin/superadmin | archive code | archive/delete | /api/dashboard/promo-codes/:id | DELETE | id | toast says archived/disabled | API error toast | Verified Runtime | 2026-07-04 verified unused archive, `409 PROMO_IN_USE`, and `includeDeleted=true`. |
...
2026-07-04 update: final focused runtime retest passed after Issue #22 seed deploy. Admin, superadmin, and courier received actionable subscription delivery rows. The UI rendered backend `allowedActions`, executed the backend-provided `PUT /api/courier/deliveries/:id/arriving-soon` action as courier, showed toast behavior, and refetched courier delivery data.
...
- Risk: No remaining P1 runtime blocker from QA-004. Continue to regression-test delivery action labels, disabled states, reason requirements, methods/endpoints, and duplicate prevention after future action-contract changes.
...
- Runtime retest needed: Completed on 2026-07-04 for rendering, no duplicates, backend endpoint/method use, toast behavior, and invalidation/refetch after a safe action.

### QA-005 Source Review
2026-07-04 update: final focused runtime retest passed after Issue #21 seed deploy and a targeted frontend archive guard fix. `PMQAARCHIVESAFE` archived successfully and appeared with `includeDeleted=true`. `PMQAARCHIVECONFLICT` returned `409 PROMO_IN_USE`, showed the existing error toast path, and remained unarchived.
...
- Risk: UI promises soft archive, while HTTP method/function name is delete. Runtime confirms backend soft archive for safe promos and `409 PROMO_IN_USE` for in-use promos.
- Backend decision needed: Closed for current contract. Keep DELETE as soft archive unless backend introduces an explicit archive endpoint later.
- Runtime retest needed: Completed on 2026-07-04 for safe archive, `includeDeleted=true`, and in-use conflict handling.
